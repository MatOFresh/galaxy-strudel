...
const MODS = [
  { id: 'room', emoji: '🏔️', label: 'Réverb', code: '.room(0.5)' },
  { id: 'delay', emoji: '🔁', label: 'Écho', code: '.delay(0.4)' },
  { id: 'lpf', emoji: '🌫️', label: 'Filtre', code: '.lpf(700)' },
  { id: 'fast', emoji: '⚡', label: 'Plus vite', code: '.fast(2)' },
  { id: 'slow', emoji: '🐢', label: 'Plus lent', code: '.slow(2)' },
  { id: 'rev', emoji: '🔀', label: 'À l\'envers', code: '.rev()' },
  { id: 'crush', emoji: '🤖', label: 'Robot', code: '.crush(6)' },
  { id: 'distort', emoji: '🔥', label: 'Disto', code: '.distort(2)' },
  { id: 'jux', emoji: '🎧', label: 'Stéréo', code: '.jux(rev)' },
  { id: 'gain', emoji: '🔉', label: 'Moins fort', code: '.gain(0.5)' },
];
...
export function createBlocks(ctx) {
  ...
  function render() {
    ...
    const add = el('button', 'kz-add-lane', '➕ Ajouter une voie');
  ...
  function renderLane(l, level) {
    ...
    sound.innerHTML = `<span class="kz-emoji">${snd ? snd.emoji : '🔊'}</span><span>${snd ? snd.label : l.soundId}</span>`;
    ...
    if (l.type !== 'melo') {
      ...
      rhythm.innerHTML = `<span class="kz-emoji">🥁</span><span>${l.rhythm}</span>`;
    ...
    l.mods.forEach((mid, idx) => {
      ...
      chip.innerHTML = `<span class="kz-emoji">${m.emoji}</span><span>${m.label}</span><span class="kz-block-x">✕</span>`;
      chip.addEventListener('click', () => { l.mods.splice(idx, 1); render(); changed(); });
      lane.append(chip);
    });

    // Bouton + effet
    const addMod = el('button', 'kz-block kz-block-add', '➕');
    addMod.addEventListener('click', () => openModPalette(l));
    lane.append(addMod);

    // Supprimer la voie
    const del = el('button', 'kz-lane-del', '🗑️');
  ...
  function openModPalette(lane) {
    ...
    MODS.forEach((m) => {
      ...
      b.innerHTML = `<span class="kz-emoji">${m.emoji}</span><span class="kz-ibl">${m.label}</span>`;
